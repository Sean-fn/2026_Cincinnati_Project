...
from llava.mm_utils import tokenizer_image_token, tokenizer_hybrid_token
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['mm_projector', 'vision_tower', 'vision_resampler', 'deepfake_projector', 'deepfake_encoder']
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    keys_to_match = []
    print(trainer.args)
    if getattr(trainer.args, "tune_mm_mlp_adapter", False) or getattr(trainer.args, "tune_deepfake_mlp_adapter", False):
        # Only save Adapter
        if getattr(trainer.args, "tune_mm_mlp_adapter", False):
            keys_to_match.extend(['mm_projector'])
        if getattr(trainer.args, "tune_deepfake_mlp_adapter", False):
            keys_to_match.extend(['deepfake_projector'])
        if getattr(trainer.args, "use_im_start_end", False):
            keys_to_match.extend(['embed_tokens', 'embed_in'])
        
        print(f'save dict keys: {keys_to_match}')
...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            if DEFAULT_DEEPFAKE_TOKEN in sentence['value']:
                sentence['value'] = sentence['value'].replace(DEFAULT_DEEPFAKE_TOKEN, '').strip()
                sentence['value'] = DEFAULT_DEEPFAKE_TOKEN + '\n' + sentence['value']
...
def preprocess_v1(
    ...
    if has_image:
        input_ids = torch.stack([tokenizer_hybrid_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations], dim=0)
    else:
        input_ids = tokenizer(
            conversations,
            return_tensors="pt",
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                round_len = len(tokenizer_hybrid_token(rou, tokenizer))
                instruction_len = len(tokenizer_hybrid_token(parts[0], tokenizer)) - 2
            else:
                round_len = len(tokenizer(rou).input_ids)
                instruction_len = len(tokenizer(parts[0]).input_ids) - 2
...
def preprocess_plain(
    ...
    # conversations = []
    # for source in sources:
    #     assert len(source) == 2    # one question and one answer
        # assert DEFAULT_IMAGE_TOKEN in source[0]['value'] or DEFAULT_DEEPFAKE_TOKEN in source[0]['value']
        # if DEFAULT_DEEPFAKE_TOKEN in source[0]['value']:
        #     source[0]['value'] = DEFAULT_DEEPFAKE_TOKEN
        # elif DEFAULT_IMAGE_TOKEN in source[0]['value']:
    ...
    # tokenize conversations
    input_ids = [tokenizer_hybrid_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations]
    targets = copy.deepcopy(input_ids)
    for target, source in zip(targets, sources):
        tokenized_len = len(tokenizer_hybrid_token(source[0]['value'], tokenizer))
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if self.data_args.h5_folder is not None:
            self.h5_datasets = {}
            for fn in os.listdir(self.data_args.h5_folder):
                data_file = re.match('FFPP_([a-zA-z0-9]*)_c40_dlibv3_lite.h5', fn)
                if data_file is not None:
                    data_fn = data_file.group(0)
                    data_name = data_file.group(1)
                    self.h5_datasets[data_name] = h5py.File(os.path.join(self.data_args.h5_folder, data_fn), 'r')

    def __len__(self):
        return len(self.list_data_dict)
    
    # What are the meanings of lengths and modality_lengths?
    @property
    def lengths(self):
        length_list = []
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            ...
            if '://' in image_file:    # load from h5
                dataset, frame_id = image_file.split('://')
                frame_key, frame_index = frame_id.rsplit('_', maxsplit=1)
                frame = self.h5_datasets[dataset][frame_key][int(frame_index), :, :, :]
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            data_dict['deepfake_inputs'] = [image]
        elif self.data_args.is_multimodal:
            raise notImplementedError("Missing key \"Image\" in conversations.")
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'deepfake_inputs' in instances[0]:
            deepfake_inputs = [instance['deepfake_inputs'] for instance in instances]
            batch['deepfake_inputs'] = deepfake_inputs
        return batch
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model = transformers.LlavaLlamaForCausalLMDeepfake.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            attn_implementation=attn_implementation,
            torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    print(f'Load model of {type(model)}')
    ...
    if training_args.lora_enable:
        ...
        lora_config = LoraConfig(
            ...
            target_modules=find_all_linear_names(model),    # mlp + decoder linear
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.tune_deepfake_mlp_adapter = training_args.tune_deepfake_mlp_adapter = model_args.tune_deepfake_mlp_adapter

        if model_args.tune_mm_mlp_adapter:
            # model.requires_grad_(False)
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = True
            print('tune mlp adapter')

        model.config.freeze_mm_mlp_adapter = training_args.freeze_mm_mlp_adapter
        if training_args.freeze_mm_mlp_adapter:
            for p in model.get_model().mm_projector.parameters():
                p.requires_grad = False
            print('freeze mlp adapter')
        if training_args.bits in [4, 8]:
    ...
    # Check if on meta device (happens with 4-bit/8-bit quantization)
    ...
    if current_is_kan != target_is_kan:
        print(f'Projector type mismatch: current_is_kan={current_is_kan}, target_is_kan={target_is_kan}, needs rebuild')
        needs_rebuild = True
    ...
    if model_args.tune_deepfake_mlp_adapter:
        for p in model.deepfake_projector.parameters():
            p.requires_grad = True
        print('tune deepfake mlp adapter')
    else:
        for p in model.deepfake_projector.parameters():
            p.requires_grad = False
        print('freeze deepfake mlp adapter')
    if training_args.bits in [4, 8]:
        model.deepfake_projector.to(dtype=compute_dtype, device=training_args.device)
